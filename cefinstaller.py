...
oms_agent_purge_command = "curl_cli --cacert /opt/CPshrd-R80.40/conf/ca-bundle.crt -O onboard_agent.sh https://raw.githubusercontent.com/Microsoft/OMS-Agent-for-Linux/master/installer/scripts/onboard_agent.sh && sh onboard_agent.sh --purge"
...
def download_omsagent():
    ...
    print_notice("curl_cli " + oms_agent_url)
    download_command = subprocess.Popen(["curl_cli", "-O", omsagent_file_name, oms_agent_url], stdout=subprocess.PIPE)
...
def check_portal_auto_sync():
    ...
    print_warning("To disable the auto sync with the portal please run: \"su omsagent -c 'python /opt/microsoft/omsconfig/Scripts/OMS_MetaConfigHelper.py --disable'\"")
